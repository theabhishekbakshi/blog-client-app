    ...
    <div>
      ...
      {isPending ? (
        ...
        <div className="flex items-center gap-2 py-2 text-sm cursor-pointer" onClick={handleSave}>
          ...
          >
            <path
              d="M12 5C10.3 5 9 6.3 9 8v36l15-9 15 9V8c0-1.7-1.3-3-3-3H12z"
              stroke="black"
              strokeWidth="2"
          ...
          {saveMutation.isPending && (
            <span className="text-xs">
              {isSaved ? "removing from saved..." : "saving..."}
            </span>
          )}
        </div>
      )}
        ...
        isAdmin && <div className="flex items-center gap-2 py-2 text-sm cursor-pointer" onClick={handleFeature}>
          ...
          >
            <path 
              ...
              stroke="black"
              strokeWidth="2"
              fill={featureMutation.isPending ?  post.isFeatured ? "none" : "gold" : post.isFeatured ? "gold" : "none"}
            />
          </svg>
          <span>{post.isFeatured ? "Featured" : "Feature this post"}</span>
          {featureMutation.isPending && (
            <span className="text-xs">
              {post.isFeatured ? "removing from featured..." : "adding to featured..."}
            </span>
          )}
        </div>
      }
      {user && (post.user.username === user.username || isAdmin) && (
        ...
        >
          ...
          {deleteMutation.isPending && (
            <span className="text-xs">(deleting in progress...)</span>
          )}
        </div>
      )}
    </div>
